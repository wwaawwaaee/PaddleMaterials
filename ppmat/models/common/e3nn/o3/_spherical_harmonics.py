...
class SphericalHarmonics(paddle.nn.Layer):
    ...
    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        if self.normalize:
            x = paddle.nn.functional.normalize(x=x, axis=-1)
            ...
            sh = sh / paddle.to_tensor(  
                paddle.concat(
                    x=[
                        (
                            math.sqrt(2 * l + 1)
                            * paddle.ones(shape=2 * l + 1, dtype=sh.dtype)
                        )
...
def _spherical_harmonics(
    lmax: int, x: paddle.Tensor, y: paddle.Tensor, z: paddle.Tensor
) -> paddle.Tensor:
    sh_0_0 = paddle.ones_like(x=x)
    if lmax == 0:
    ...
    if lmax == 2:
        return paddle.stack(
            x=[sh_0_0, sh_1_0, sh_1_1, sh_1_2, sh_2_0, sh_2_1, sh_2_2, sh_2_3, sh_2_4],
            axis=-1,
        )
...
def _generate_spherical_harmonics(lmax, device=None):
    """code used to generate the code above

    based on `wigner_3j`
    """
    paddle.set_default_dtype(d="float64")
