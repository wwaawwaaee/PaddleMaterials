...
def soft_one_hot_linspace(
    x: paddle.Tensor, start, end, number, basis=None, cutoff=None
):
    ...
    if basis == "gaussian":
        ...
        mask = paddle.cast((diff < 1) & (-1 < diff), dtype=x.dtype)
        return paddle.cos(x=math.pi / 2 * diff) * mask
    if basis == "smooth_finite":
        return (
            1.14136
            * paddle.exp(x=paddle.to_tensor(data=2.0))
            * soft_unit_step(diff + 1)
            * soft_unit_step(1 - diff)
        )
    if basis == "fourier":
