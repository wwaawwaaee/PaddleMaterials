...
# Offer higher-level utilities for model trainig and management.
"""
...
import paddle
from paddle.fft import fftn
from paddle.fft import fftshift
from paddle.fft import ifftn
from paddle.fft import ifftshift
...
def randn_tensor(
    ...
    *kwargs,
    ...
    passing a list of generators, you can seed each batch size individually. If CPU
    generators are passed, the tensor
    ...
    if isinstance(generator, (list, tuple)):
        ...
        latents = [
            randn_pt(shape, generator=generator[i], dtype=dtype)
...
def rand_tensor(
    ...
    *kwargs,
    ...
    """A helper function to create random tensors with the desired `dtype`. When
    passing a list of generators, you can seed each batch size individually. If CPU
    generators are passed, the tensor is always created on the CPU.
    """
    # make sure generator list of length 1 is treated like a non-list
    if isinstance(generator, list) and len(generator) == 1:
        generator = generator[0]
    ...
    if isinstance(generator, (list, tuple)):
        ...
        latents = [
            rand_pt(shape, generator=generator[i], dtype=dtype)
            for i in range(batch_size)
        ]
        latents = paddle.concat(latents, axis=0)
    else:
...
def randint_tensor(
    ...
    *kwargs,
    ...
    """This is a helper function that allows to create random tensors on the desired
    `device` with the desired `dtype`. When passing a list of generators one can seed
    each batched size individually. If CPU generators are passed the tensor
...
def no_init_weights(_enable=True):
    """
    Context manager to globally disable weight initialization to speed up loading large
    models.
...
def apply_freeu(
    ...
    **freeu_kwargs,
    ...
    Args:
        resolution_idx (`int`): Integer denoting the UNet block where FreeU is being
            applied.
        hidden_states (`paddle.Tensor`): Inputs to the underlying block.
        res_hidden_states (`paddle.Tensor`): Features from the skip block corresponding
            to the underlying block.
        s1 (`float`): Scaling factor for stage 1 to attenuate the contributions of the
            skip features.
        s2 (`float`): Scaling factor for stage 2 to attenuate the contributions of the
            skip features.
        b1 (`float`): Scaling factor for stage 1 to amplify the contributions of
            backbone features.
        b2 (`float`): Scaling factor for stage 2 to amplify the contributions of
            backbone features.
    """
    if resolution_idx == 0:
